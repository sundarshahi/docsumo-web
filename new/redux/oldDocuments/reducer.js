...
function getReviewToolDocInitialState() {
  return {
...
export default handleActions(
  {
    ...
    [actionTypes.RT_LINE_ITEMS_DELETE_FIELDS](state, { payload }) {
      const { rowFieldIds, sectionFieldId } = payload;

      const existingDeletingFieldIds =
        state[REVIEW_TOOL].deletingLineItemFieldIds || [];
      const newDeletingFieldIds = [...existingDeletingFieldIds, ...rowFieldIds];

      let fieldsById = {
        ...state[REVIEW_TOOL].fieldsById,
      };

      let selectedFieldById = fieldsById[sectionFieldId];
      selectedFieldById = selectedFieldById ? selectedFieldById.children : [];
      let indexes = rowFieldIds.map((rowField) =>
        selectedFieldById.findIndex(
          (selctedField) =>
            selctedField.findIndex((x) => x.id === rowField) !== -1
        )
      );
      indexes = _.union(indexes);
      indexes.forEach((rowFieldId) => {
        if (_.has(selectedFieldById, rowFieldId)) {
          delete selectedFieldById[rowFieldId];
        }
      });

      fieldsById = {
        ...fieldsById,
        children: selectedFieldById,
      };

      newDeletingFieldIds.forEach((rowFieldId) => {
      ...
      return {
        ...
        [REVIEW_TOOL]: {
          ...state[REVIEW_TOOL],
          fieldsById,
          deletingLineItemFieldIds: newDeletingFieldIds,
        },
      };
    },
    ...
    [actionTypes.RT_LINE_ITEMS_DELETE_FIELDS_FULFILLED](state, { meta }) {
      const { sectionFieldId, rowIds, rowFieldIds } = meta;

      const deletingLineItemFieldIds = _.difference(
        state[REVIEW_TOOL].deletingLineItemFieldIds,
        rowFieldIds
      );
      ...
      rowFieldIds.forEach((rowFieldId) => {
        if (_.has(rowFieldIds, rowFieldId)) {
          delete fieldsById[rowFieldId];
        }
      });
      ...
      rowIds.forEach((rowIds) => {
        if (_.has(lineItemRowsById, rowIds)) {
          delete lineItemRowsById[rowIds];
        }
      });
    ...
    [actionTypes.RT_FIELD_DATA_PERSISTANCE_FULFILLED](state, { payload }) {
      const { docId, fieldId, response } = payload;
      ...
      const content = _.get(
        data,
        'item.content',
        state[REVIEW_TOOL].fieldsById[fieldId].content
      );
      const now = _.now();
      return {
        ...state,
    ...
    [actionTypes.HANDLE_GRID_DRAG](state, { payload }) {
