...
class RowDivider extends Component {
  ...
  shouldComponentUpdate(_nextProps, nextState) {
    ...
    const { atLeastOneHeaderFound, gridView } = this.props;
    if (
      top !== preTop ||
      ignore !== prevIgnore ||
      height !== prevHeight ||
      ...
      <>
        {gridView && absolute ? (
          <>
            ...
            ) : atLeastOneHeaderFound && !isHeaderSelected ? (
              <button
                className={cx(styles.addRowHeader, {
                  [styles.rowHeaderSelected]: postRow && isHeaderSelected,
                })}
          ...
          <>
            {isHeaderSelected ? (
              <button
                className={cx(
                  styles.addRowHeader,
                  styles.passiveAddHeaderRow,
                  styles.passiveAddHeaderRowSelected
                )}
                onClick={() => {
                  selectHeader(!isHeaderSelected);
                }}
        ...
        >
          {gridView ? (
            <>
              <HelpTooltip
                id={
                  absolute && ttConstants.TT_REVIEW_SCRREN_GRID_IGNORE_ROW_DATA
                }
              >
                <button
                  className={styles.removeImgBox}
                  onClick={() => {
                    if (absolute && isHeaderSelected) {
                      return null;
                    }
                    onRemove(!ignore);
                  }}
