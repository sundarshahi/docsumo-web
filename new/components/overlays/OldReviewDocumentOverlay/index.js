...
class ReviewDocumentOverlay extends Component {
  ...
  handleCloseGrid = () => this.setState({ gridView: false });
  ...
  generateLocationList = () => {
    ...
    sectionFieldIds.forEach((sectionFieldId) => {
      ...
      if (sectionField.type === documentConstants.FIELD_TYPES.LINE_ITEM) {
        ...
        const rowIds = sectionField.lineItemRowIds;
        rowIds &&
  ...
  focusDomElementById = (elementId, tryCount = 1) => {
    ...
    if (domEl) {
      /**
       * FIXME: This is just for test purpose to fix issue of field scrolling to bottom.
       * If any other issue arises by commenting this code then we have to uncomment this code
       * Otherwise, delete this code before releasing
       */
  ...
  handleFieldDataUpdateRequest = async (fieldData, throwErrorOnFailure) => {
    ...
    const { documentActions, rtUpdateFields } = this.props;
    const { slug } = this.state;
    try {
  ...
  handleFooterAddLineBtnClick = ({ sectionFieldId }) => {
    ...
    setTimeout(async () => {
      const response = await api.getDocumentDataV1({
        docId,
      });
  ...
  handleFooterDeleteAllRowsBtnClick = ({
    sectionFieldId,
    rowIds,
    rowFieldIds,
  }) => {
  ...
  handleFooterDeleteRowBtnClick = ({ sectionFieldId, rowId, rowFieldIds }) => {
    ...
    if (rowFieldIds.includes(this.props.selectedFieldId)) {
      const { selectedSectionField, lineItemRowsById } = this.props;
      const { lineItemRowIds } = selectedSectionField;
      const rowIndex = lineItemRowIds.indexOf(rowId);
      const prevRowId = lineItemRowIds[rowIndex - 1];
      const nextRowId = lineItemRowIds[rowIndex + 1];
    ...
    !isLabelSelected
      ...
      : this.props.documentActions.rtUpdateFieldData({
          ...
          updates: {
            ...
            // uiLabelRectangle: rectangle,
            // uiLabelRectanglePercentages: rectanglePercentages,
            // uiLabelPosition: position,
          },
        });
  ...
  handleConfirmFinishReviewBtnClick = async () => {
    ...
    try {
      ...
      const dataResponse = await api.getDocumentDataV1({
        docId,
      });
  ...
  handleFetchRetryBtnClick = () => {
    this.props.documentActions.rtgetDocumentDataV1({
      docId: this.props.docId,
    });
  };
  ...
  render() {
    ...
    const footerDefaultProps = {
      ...
      onDeleteRowBtnClick: this.handleFooterDeleteRowBtnClick,
    ...
    return (
      <>
        ...
        {/* <SpreadsheetOverlay documentTitle = { docMeta && docMeta.title || ''} docId = { docId || ''}/> */}
        {reviewConfirmation ? (
          <ConfirmationModal
            title={'Save Changes'}
