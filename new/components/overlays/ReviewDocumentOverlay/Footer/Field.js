...
class Field extends Component {
  ...
  handleInputFocus = () => {
    ...
    if (bboxClickType !== 'field_click') {
      handleBboxViewer('field_click');
    }

    handleTableFieldTrackingStart({
      name: field._parentId,
      fieldLabel: field.subPTitle,
      valueToSave: field,
  ...
  renderSelectInputField = () => {
    ...
    const tableWrapperRoot = document.getElementById('gridTableWrapper');
          ...
          {!isFieldSelected ? (
            ...
            <OutsideClickHandler onOutsideClick={this.closeDropDown}>
              <div className={cx(selectBoxStyles.root)}>
                <InView
                  as='div'
                  root={tableWrapperRoot}
                  rootMargin='-95px 0px 0px 0px'
                  triggerOnce={false}
                  onChange={(inView) => {
                    this.setState({
                      isDropdownVisible: inView,
                    });
                  }}
                >
                  <textarea
                ...
                {docReadOnly ? (
                  ...
                  <ul
                    className={cx(selectBoxStyles.list, {
                      'd-none': !this.state.isDropdownVisible,
                    })}
                    id={`line-item-field-dropdown-${field.id}`}
                    style={{
                      width: this.inputFieldRef.current?.offsetWidth,
                    }}
                  >
                    {loading ? (
  ...
  render() {
    const { field, isFieldSelected, column, wrapItem, isDeletingRow } =
      this.props;
