...
class AllActivityPage extends Component {
  state = {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextLocation.state && nextLocation.state.forceReload) {
      history.replace(routes.ALL_ACTIVITY);
      this.updateParamStateKeys({
        pageQueryParams: {},
      });
      this.fetchDocuments({});
  ...
  componentWillUnmount() {
    this.props.activityActions.activityReset();
  }
  ...
  fetchDocuments = (pageQueryParams) => {
    ...
    activityActions.allActivityFetch({
      queryParams,
    });
  };
  ...
  renderDescriptionBody = ({
    ...
    return (
      <>
        ...
        {mode === 'test' && <Badge type='warning' title='Test Mode' />}
        {downloadUrl && (
          <Button
            size='small'
            variant='outlined'
  ...
  renderDocumentList = ({ showSkeletonView }) => {
    const { allActivity, appActions } = this.props;
  ...
  render() {
    const { isFetchingDocuments, fetchFailed, fetchSucceeded, allActivity } =
      this.props;

    const hasDocuments = !_.isEmpty(allActivity);
    ...
    return (
      <Fragment>
        <PageMetadata title='All Activity' />
...
function mapStateToProp(state) {
  const { allActivity, meta, fetchState } = state.activities.allActivity;
...
export default connect(mapStateToProp, mapDispatchToProps)(AllActivityPage);
