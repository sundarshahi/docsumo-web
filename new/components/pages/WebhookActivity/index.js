...
import { WebhookListHeader } from './WebhookActivityList';

import styles from './Webhook.scss';

class WebhookActivityPage extends Component {
  state = {
  ...
  componentDidMount() {
    ...
    this.fetchActivities(pageQueryParams);
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextLocation.state && nextLocation.state.forceReload) {
      history.replace(routes.WEBHOOK);
      this.updateParamStateKeys({
        pageQueryParams: {},
      });
    ...
    if (paramsChanged) {
      ...
      this.fetchActivities(nextPageQueryParams);
    }
  }
  ...
  fetchActivities = (pageQueryParams) => {
    ...
    activityActions.webhookActivityFetch({
      queryParams,
    });
  };
  ...
  renderActivityList = ({ showSkeletonView }) => {
    const { webhookActivity } = this.props;
  ...
  render() {
    ...
    const hasDocuments = !_.isEmpty(webhookActivity);
    ...
    return (
      <Fragment>
        <PageMetadata title='Webhook Activity' />
        ...
        {this.renderActivityList({ showSkeletonView })}
...
function mapStateToProp(state) {
  const { webhookActivity, meta, fetchState } =
    state.activities.webhookActivity;
...
export default connect(mapStateToProp, mapDispatchToProps)(WebhookActivityPage);
