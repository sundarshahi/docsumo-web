...
class SkippedDocumentsPage extends Component {
  state = {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextLocation.state && nextLocation.state.forceReload) {
      history.replace(
        folder_id ? `${routes.SKIPPED}?folder_id=${folder_id}` : routes.SKIPPED
      );
      this.updateParamStateKeys({
        pageQueryParams: {},
  ...
  componentWillUnmount() {
    this.props.documentActions.skippedDocumentsReset();
  }
  ...
  fetchDocuments = (pageQueryParams) => {
    ...
    const { q = '' } = searchQuery;
    this.props.documentActions.skippedDocumentsFetch({
      queryParams: {
        q,
        ...pageQueryParams,
        view: 'folder',
    ...
    switch (action) {
      ...
      case 'folderView': {
        (async () => {
          ...
          history.push(`${routes.SKIPPED}?${queryString.stringify(params)}`);
        })();
      }
    }
...
function mapStateToProp(state) {
  ...
  const {
    ...
    selectedAll,
    slectedList = [],
  } = state.documents.skippedDocumentsPage;
