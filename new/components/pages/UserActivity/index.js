...
class UserActivityPage extends Component {
  ...
  componentWillUnmount() {
    this.props.activityActions.userActivityReset();
  }
  ...
  fetchDocuments = (pageQueryParams) => {
    ...
    activityActions.userActivityFetch({
      queryParams,
    });
  };
  ...
  renderDocumentList = ({ showSkeletonView }) => {
    const { userActivity, appActions } = this.props;
  ...
  render() {
    const { isFetchingDocuments, fetchFailed, fetchSucceeded, userActivity } =
      this.props;

    const hasDocuments = !_.isEmpty(userActivity);
    ...
    return (
      <Fragment>
        <PageMetadata title='User Activity' />
...
function mapStateToProp(state) {
  const { userActivity, meta, fetchState } = state.activities.userActivity;
...
export default connect(mapStateToProp, mapDispatchToProps)(UserActivityPage);
